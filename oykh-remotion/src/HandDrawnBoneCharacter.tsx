...
interface HandDrawnBoneCharacterProps {
  pose: Pose;
  x?: number;
  y?: number;
  scale?: number;
...
export const HandDrawnBoneCharacter: React.FC<HandDrawnBoneCharacterProps> = ({
  pose,
  x = 960,
  y = 540,
  scale = 1,
  ...
  const lElbow = pt(lShoulder.x, lShoulder.y, 180 - pose.leftShoulder, upperArmLen);
  const lHand = pt(lElbow.x, lElbow.y, 180 - pose.leftShoulder - pose.leftElbow, lowerArmLen);
  ...
  const lKnee = pt(lHip.x, lHip.y, 180 + pose.leftHip, upperLegLen);
  const lFoot = pt(lKnee.x, lKnee.y, 180 + pose.leftHip - pose.leftKnee, lowerLegLen);
    ...
    <svg width={1920} height={1080} style={{ position: 'absolute', left: 0, top: 0 }}>
      <defs>
        ...
        <filter id="shadow" x="-50%" y="-50%" width="200%" height="200%">
          ...
          <feOffset dx="5" dy="8" result="shadow"/>
          <feFlood floodColor="#1a1a2e" floodOpacity="0.3"/>
      ...
      <g transform={`translate(${x}, ${y}) scale(${scale})`}>
        {/* Ground shadow */}
        <ellipse cx="0" cy={hipY + upperLegLen + lowerLegLen + 50} rx="150" ry="20" fill="rgba(30, 30, 50, 0.4)"/>

        <g filter="url(#shadow) url(#sketchy)">
          {/* === LEGS (behind body) === */}
          {/* Right thigh */}
          ...
          <path d={capsule(rKnee.x, rKnee.y, rFoot.x, rFoot.y, legR * 0.9)}
                ...
                strokeLinejoin="round" strokeLinecap="round"/>
          {/* Right foot */}
          <circle cx={rFoot.x} cy={rFoot.y} r={footR}
                  fill={fill} stroke={stroke} strokeWidth={strokeWidth}/>
          ...
          <path d={capsule(lKnee.x, lKnee.y, lFoot.x, lFoot.y, legR * 0.9)}
                ...
                strokeLinejoin="round" strokeLinecap="round"/>
          {/* Left foot */}
          <circle cx={lFoot.x} cy={lFoot.y} r={footR}
                  fill={fill} stroke={stroke} strokeWidth={strokeWidth}/>
          ...
          <path d={capsule(lElbow.x, lElbow.y, lHand.x, lHand.y, armR * 0.88)}
                ...
                strokeLinejoin="round" strokeLinecap="round"/>
          {/* Left hand */}
          <circle cx={lHand.x} cy={lHand.y} r={handR}
                  fill={fill} stroke={stroke} strokeWidth={strokeWidth}/>
          ...
          <path d={capsule(rElbow.x, rElbow.y, rHand.x, rHand.y, armR * 0.88)}
                ...
                strokeLinejoin="round" strokeLinecap="round"/>
          {/* Right hand */}
          <circle cx={rHand.x} cy={rHand.y} r={handR}
                  fill={fill} stroke={stroke} strokeWidth={strokeWidth}/>
          ...
          {/* Eye highlights */}
          <circle cx="-31" cy="0" r="4" fill="#FFFFFF" opacity="0.95"/>
          <circle cx="25" cy="0" r="4" fill="#FFFFFF" opacity="0.95"/>
        </g>
      </g>
    </svg>
  );
...
// Animated version
interface AnimatedHandDrawnBoneCharacterProps {
...
export const AnimatedHandDrawnBoneCharacter: React.FC<AnimatedHandDrawnBoneCharacterProps> = ({
  poseSequence,
  x = 960,
  y = 540,
  scale = 1,
  ...
  return <HandDrawnBoneCharacter pose={currentPose} x={x} y={y + bob} scale={scale}/>;
};
